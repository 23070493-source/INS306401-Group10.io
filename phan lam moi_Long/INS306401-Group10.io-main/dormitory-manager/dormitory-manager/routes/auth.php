...
$routes['login'] = function (PDO $db): void {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($username === '' || $password === '' || $roleName === '') {
            $error = 'Vui lòng nhập đầy đủ vai trò, tên đăng nhập và mật khẩu.';
        } elseif (!in_array($roleName, ['Admin', 'Manager', 'Student'], true)) {
            $error = 'Vai trò đăng nhập không hợp lệ.';
        } elseif (Auth::attempt($username, $password, $roleName)) {
            redirectTo(Auth::dashboardRoute());
        } else {
            $error = 'Tài khoản không tồn tại hoặc không khớp vai trò đã chọn.';
        }
    }
