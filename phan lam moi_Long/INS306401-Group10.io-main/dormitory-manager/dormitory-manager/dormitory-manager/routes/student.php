...
if (!function_exists('studentEnsurePaymentSchema')) {
    function studentEnsurePaymentSchema(PDO $db): void
    {
        studentAddColumnIfMissing($db, 'payments', 'payment_proof_image', 'payment_proof_image VARCHAR(255) NULL');
        studentAddColumnIfMissing($db, 'payments', 'gateway_status', "gateway_status VARCHAR(50) NULL DEFAULT 'manual_pending'");
        studentAddColumnIfMissing($db, 'payments', 'gateway_transaction_id', 'gateway_transaction_id VARCHAR(100) NULL');
        studentAddColumnIfMissing($db, 'payments', 'transfer_content', 'transfer_content VARCHAR(255) NULL');
    }
}

if (!function_exists('studentStorePaymentProof')) {
    function studentStorePaymentProof(array &$errors): ?string
    {
        if (empty($_FILES['payment_proof_image']) || ($_FILES['payment_proof_image']['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_NO_FILE) {
            return null;
        }

        $file = $_FILES['payment_proof_image'];

        if (($file['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK) {
            $errors[] = 'Không tải được ảnh bill chuyển khoản. Vui lòng chọn lại ảnh.';
            return null;
        }

        if (($file['size'] ?? 0) > 5 * 1024 * 1024) {
            $errors[] = 'Ảnh bill chuyển khoản không được vượt quá 5MB.';
            return null;
        }
        ...
        if (!isset($allowedMimeTypes[$mimeType])) {
            $errors[] = 'Ảnh bill chuyển khoản chỉ nhận JPG, PNG hoặc WEBP.';
            return null;
        }

        $uploadDir = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'payment_proofs';

        if (!is_dir($uploadDir) && !mkdir($uploadDir, 0775, true) && !is_dir($uploadDir)) {
            $errors[] = 'Không tạo được thư mục lưu ảnh bill chuyển khoản.';
            return null;
        }

        $fileName = 'payment_' . date('YmdHis') . '_' . bin2hex(random_bytes(4)) . '.' . $allowedMimeTypes[$mimeType];
        $destination = $uploadDir . DIRECTORY_SEPARATOR . $fileName;

        if (!move_uploaded_file($file['tmp_name'], $destination)) {
            $errors[] = 'Không lưu được ảnh bill chuyển khoản. Vui lòng thử lại.';
            return null;
        }
...
$routes['student/my-invoices'] = function (PDO $db): void {
    ...
    if ($student) {
        $stmt = $db->prepare("
            ...
            JOIN buildings b ON b.id = r.building_id
            WHERE (i.student_id = :invoice_student_id OR c.student_id = :contract_student_id)
            ORDER BY
                CASE i.status
                    WHEN 'overdue' THEN 1
                    WHEN 'unpaid' THEN 2
        ...
        $stmt->execute([
            'invoice_student_id' => $student['id'],
            'contract_student_id' => $student['id']
        ]);
...
$routes['student/invoice-print'] = function (PDO $db): void {
    ...
    $stmt = $db->prepare("
        ...
        WHERE i.id = :invoice_id
          AND (i.student_id = :invoice_student_id OR c.student_id = :contract_student_id)
        LIMIT 1
    ");

    $stmt->execute([
        'invoice_id' => $invoiceId,
        'invoice_student_id' => $studentId,
        'contract_student_id' => $studentId
    ]);
...
$routes['student/payment-store'] = function (PDO $db): void {
    ...
    $allowedMethods = ['bank_transfer', 'cash'];
    ...
    try {
        ...
        if ($paymentMethod === 'bank_transfer') {
            $proofImagePath = studentStorePaymentProof($errors);

            if ($proofImagePath === null) {
                $errors[] = 'Vui lòng upload ảnh minh chứng bill chuyển khoản.';
            }
        }

        if (!empty($errors)) {
            render('student/payment_submit', [
                'title' => 'Gửi thông tin thanh toán',
                'student' => $student,
                'invoice' => $invoice,
        ...
        if ($paymentMethod === 'cash') {
            ...
            $transactionReference = 'CASH-' . date('YmdHis') . '-' . $invoiceId;
            $proofImagePath = null;
            $gatewayStatus = 'cash_pending';
            $gatewayTransactionId = null;
            $note = $note !== '' ? $note : 'Student will pay directly at school office.';
        } else {
            $gatewayStatus = 'proof_submitted';
            $gatewayTransactionId = null;
        }
        ...
        $newValue = $paymentMethod === 'cash'
            ? 'Student submitted cash payment request for invoice ' . $invoice['invoice_code']
            : 'Student submitted bank transfer proof for invoice ' . $invoice['invoice_code'];
        ...
        echo '<h2>Gửi thông tin thanh toán thất bại</h2>';
