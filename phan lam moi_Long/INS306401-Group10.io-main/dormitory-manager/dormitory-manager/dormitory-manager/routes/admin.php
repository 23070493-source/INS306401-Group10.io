...
$routes['admin/violations'] = function (PDO $db): void {
    Auth::requireRole('Admin');
    ...
    $violations = $db->query("
        ...
        LEFT JOIN users recorder ON recorder.id = vr.recorded_by
        ORDER BY
    ...
    $summary = [
        ...
        'critical_students' => array_reduce(
            $warningStudents,
            fn ($count, $student) => $count + (((int) $student['total_points'] >= 15) ? 1 : 0),
            0
        ),
        'total_points' => $db->query("SELECT COALESCE(SUM(penalty_points), 0) FROM violation_records")->fetchColumn(),
    ];

    render('admin/violations', [
        'title' => 'Vi phạm',
